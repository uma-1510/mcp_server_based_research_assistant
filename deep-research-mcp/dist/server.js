...
async function summarizeFindings(findings, query) {
    ...
    try {
        if (anthropicKey) {
            ...
            return msg.content?.[0]?.type === 'text' ? msg.content[0].text : undefined;
        }
        if (openaiKey) {
...
async function deepResearch(query, opts) {
    ...
    const pages = (await Promise.all(results
        .slice(0, maxPages)
        .map((r) => limit(() => fetchPage(r.url).catch(() => ({ url: r.url, title: r.title })))))).filter(Boolean);
...
async function main() {
    ...
    const { McpServer, ResourceTemplate } = mcpMod;
    const { StdioServerTransport } = stdioMod;
    const server = new McpServer({ name: 'deep-research-mcp', version: '0.1.0' }, {
        capabilities: { tools: {}, resources: {}, prompts: {}, logging: {} },
        instructions: 'Use the tools to search and compile deep research with citations.',
    });
    server.tool('search_web', 'Search the web for a query and return top results', {
    ...
    server.tool('fetch_url', 'Fetch and extract readability content from a URL', { url: z.string().url() }, async (args) => {
        const { url } = args;
    ...
    server.tool('deep_research', 'Perform multi-source research and produce a markdown summary with citations', {
        query: z.string().min(3),
        maxResults: z.number().int().min(1).max(25).optional(),
        maxPages: z.number().int().min(1).max(15).optional(),
        concurrency: z.number().int().min(1).max(8).optional(),
    }, async (args) => {
        const { query, maxResults, maxPages, concurrency } = args;
    ...
    server.registerResource('research-files', new ResourceTemplate('file:///workspace/deep-research-mcp/research/{name}', {
        list: async () => {
            const fs = await import('node:fs/promises');
